...
module.exports = async () => {
  ...
  try {
    ...
    nova = await newContract(TestToken, 'NovaToken', 'NOVA', 18);
    console.log('Nova Token address', web3.toChecksumAddress(nova._address));

    proxy = await newContract(Proxy);
    console.log('Proxy address', web3.toChecksumAddress(proxy._address));

    exchange = await newContract(HybridExchange, proxy._address, nova._address);
    console.log(
    ...
    await Promise.all(
      testAddresses.map(async u => {
        await approveAllToken(u, [usd, nova, weth]);
        await giveCoinsTo(u, '100000', [nova, usd]);
        await wrapETH(u, '1000');
      })
    );

    await approveAllToken(maker, [usd, nova, weth]);
    await giveCoinsTo(maker, '100000', [nova, usd]);
    await wrapETH(maker, '1000');
    await approveAllToken(relayer, [usd, nova, weth]);
